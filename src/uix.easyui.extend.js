(function ($) {
	...
	const patches = {
		...
		linkbutton: function () {
			$.fn.linkbutton = pepi($.fn.linkbutton, function () {
				...
				let cls = options.theme; //按钮主题皮肤
				if (cls) {
					uix.assignClass($target, cls);
				}
			});
		...
		// 下拉面板，实现效果点击箭头时，有动画
		combo: function () {
			//showPanelWhenInput是自定义属性，表示当有键盘输入时，是否触发显示下拉面板
			$.extend($.fn.combo.defaults, {
				showPanelWhenInput: true
			});

			//以下部分是添加一个showPanelWhenInput自定义属性，来控制下拉面板是在输入时是否触发显示
		...
		/**
		 * 窗口组件
		 * 功能增强：实现拖动时，本地位置直接跟随鼠标移动，而不是移动一个虚线框
		 */
		window: function () {
			//扩展window的默认属性
	...
	if ($.parser) {
		...
		$.parser.plugins.push("messager");
	}
	$.parser.plugins.forEach(it => patchPlugin(it));
